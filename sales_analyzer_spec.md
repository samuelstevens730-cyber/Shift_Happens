...
### Exports required
```typescript
// StoreTotals must include shiftCount so factors are based on avg-per-shift
// (not raw totals), which insulates normalization from staffing asymmetries.
interface StoreTotals {
  storeId: string
  totalSalesCents: number
  shiftCount: number  // only shifts with non-null sales data
}

// Given per-store totals, compute per-store scaling factors.
// factor = networkAvgSalesPerShift / thisStoreAvgSalesPerShift
computeScalingFactors(storeTotals: StoreTotals[]): Map<string, number>

// Apply the pre-computed factor for a store to a raw sales value.
// Returns adjusted cents (integer, rounded). Falls back to rawSalesCents if no factor.
applyScalingFactor(rawSalesCents: number, storeId: string, factors: Map<string, number>): number
```
...
interface ShiftSummary {
  ...
  startedAt: Date
  endedAt: Date
  shiftHours: number         // 0 if endedAt is null (open/in-progress shifts)
  rawSalesCents: number
  adjustedSalesCents: number
  rawPerHour: number
  adjustedPerHour: number
  performanceFlag: 'HIGH' | 'LOW' | 'NORMAL'  // ±20% vs employee avg for period (two-pass: compute avg first, then flag)
  isCountable: boolean   // true when: ended_at IS NOT NULL AND salesCents IS NOT NULL
                         // Shifts with null endedAt or null salesCents are excluded from all sales averages
}

// NOTE: performanceFlag requires a two-pass computation:
//   Pass 1 — compute all shift sales, collect adjustedSalesCents per employee
//   Compute avgAdjustedPerShift from countable shifts
//   Pass 2 — assign flag: HIGH if > 1.2× avg, LOW if < 0.8× avg, else NORMAL
//
// NOTE: shifts with ended_at = null (still in progress) are included in totalShifts
//   but excluded from sales metrics (isCountable = false, no flag assigned).
```
...
interface EmployeePeriodSummary {
  employeeId: string
  employeeName: string
  primaryStore: string          // store where majority of shifts worked; tie-break: higher total adjusted sales; further tie: alphabetical by store name
...
### Schema
```sql
-- NOTE: codebase uses `profiles` table for employees and `auth.users` for auth users.
-- UNIQUE constraint uses two partial indexes because PostgreSQL treats NULL != NULL,
-- so a standard UNIQUE constraint on nullable store_id would silently allow duplicates
-- for all-store snapshots (where store_id IS NULL).
CREATE TABLE performance_snapshots (
  ...
  -- No inline UNIQUE — handled by partial indexes below
);

-- Separate partial indexes handle nullable store_id correctly
CREATE UNIQUE INDEX performance_snapshots_unique_all_stores
  ON performance_snapshots(employee_id, period_from, period_to)
  WHERE store_id IS NULL;

CREATE UNIQUE INDEX performance_snapshots_unique_per_store
  ON performance_snapshots(employee_id, store_id, period_from, period_to)
...
interface PeriodDelta {
  ...
  adjAvgDeltaCents: number      // current - previous (CENTS — convert to dollars in formatter only)
  adjAvgDeltaPct: number        // % change
  gapVsBenchmarkDeltaCents: number  // is the gap closing or widening? (CENTS)

  highFlagDelta: number         // change in HIGH flag count
  lowFlagDelta: number          // change in LOW flag count

  trending: 'UP' | 'DOWN' | 'FLAT'   // based on adjAvgDeltaCents threshold (suggest ±2500 cents = ±$25)
...
If they diverge by more than 500 cents ($5), flag as a data quality warning in the report. This surfaces manual entry issues, missing shifts, or anything that slipped through between periods.
...
Two export formats, zero extra dependencies:

### 1. Copy as Text (for AI analysis)
- Button in UI copies the `format=text` formatter output to clipboard
- User pastes directly into Claude/ChatGPT/Gemini for analysis
- Handled by `performanceReportFormatter.ts` (already in spec)

### 2. Print / Save as PDF (for manager review)
- Button triggers `window.print()`
- UI page includes print-specific CSS (`@media print`) that hides controls and formats the report cleanly for printing
- Browser's "Save as PDF" option produces a clean printable document
- Zero library dependencies

### Future (if needed)
- Flat CSV of shift-level data (one row per shift) can be added as a separate endpoint if spreadsheet manipulation is needed
...
### Layout
- **Header**: "Sales Performance Report" + description
- **Controls bar**: date range picker, store filter, employee filter, report type selector, "Generate Report" button, "Save Snapshot" checkbox
- **Loading / error states**
- **Results** (rendered after generate):
  - Period summary header (dates, benchmark, store normalization factor)
  - Per-employee cards, sorted by adjusted avg descending:
    - Header: name, primary store, grade-style indicator (HIGH/NORMAL/LOW trend)
    - Key metrics row: Adj Avg | Raw Avg | Adj/Hr | vs Benchmark | Streak
    - Flag counts: HIGH / LOW / NORMAL
    - Expandable: shift-by-shift table with per-shift flag colors
    - Expandable: by shift type + by day of week breakdowns
    - If delta available: "vs last period" row
- **Export bar** (bottom or top of results):
  - "Copy Report (Text)" — copies formatter output to clipboard
  - "Print / Save PDF" — triggers window.print()

### Print CSS
`@media print` should:
- Hide the controls bar and export bar
- Show all expanded sections (shift details, breakdowns)
- Use a clean single-column layout
- Include page breaks between employees

### Notes
- `saveSnapshot` is passed as a query param on the GET endpoint (pragmatic, not REST-pure — acceptable for this use case)
- The benchmark employees can be configured via a collapsible "Advanced options" section in the UI (passed as comma-separated IDs or selected from the employee dropdown)
