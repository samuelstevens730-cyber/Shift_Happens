...
export async function GET(req: Request) {
  try {
    ...
    if (scopedStoreIds.length === 0) {
      const empty: EmployeePublicScoreboardResponse = {
        stores: [],
        publicRows: [],
        myRow: null,
        managerRows: [],
    ...
    const [storesRes, shiftsRes, salesRes, drawerCountsRes, closeoutsRes, cleaningRes, schedulesRes, scheduledShiftsRes, profilesRes, storeManagersRes] =
      await Promise.all([
        supabaseServer
          .from("stores")
        ...
        supabaseServer
          ...
          .in("store_id", activeStoreIds)
          .returns<Array<{ store_id: string; user_id: string }>>(),
      ]);
    ...
    const managerUserIds = Array.from(new Set((storeManagersRes.data ?? []).map((row) => row.user_id)));
    const { data: managerProfiles, error: managerProfilesErr } = managerUserIds.length
      ? await supabaseServer
          .from("profiles")
          .select("id,name,auth_user_id")
          .in("auth_user_id", managerUserIds)
          .returns<Array<{ id: string; name: string | null; auth_user_id: string | null }>>()
      : { data: [], error: null as { message: string } | null };
    if (managerProfilesErr) {
      return NextResponse.json({ error: managerProfilesErr.message }, { status: 500 });
    }
    ...
    const response: EmployeePublicScoreboardResponse = {
