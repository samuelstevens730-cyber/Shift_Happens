...
export async function GET(req: Request) {
  try {
    ...
    const scopedStoreIds = auth.storeIds ?? [];
    if (scopedStoreIds.length === 0) {
      const empty: ShiftBreakdownResponse = {
    ...
    if (storeId && storeId !== "all" && !scopedStoreIds.includes(storeId)) {
      return NextResponse.json({ error: "Invalid store filter." }, { status: 403 });
    }
    const activeStoreIds = storeId && storeId !== "all" ? [storeId] : scopedStoreIds;
    ...
    // Compute store adjustment factors (same logic as scoreboard)
    ...
    // Closeout variance keyed by shift_id (or profile fallback)
    const closeoutByShiftId = new Map<string, number>();
    const closeoutsByDate = new Map<string, number[]>();
    for (const row of closeoutsRes.data ?? []) {
      const absVariance = Math.abs(row.variance_cents ?? 0);
      if (row.shift_id) {
        closeoutByShiftId.set(row.shift_id, absVariance);
      } else {
        // No shift_id — bucket by date of the closeout for later matching
        const dateKey = businessDateFromIso(row.profile_id); // placeholder; will use completed_at instead
        // We can't reliably link these without a shift_id, so skip for per-shift view
        void dateKey;
        void closeoutsByDate;
      }
    }

    // Cleaning tasks keyed by shift_id (or by date for unlinked tasks)
    ...
    // Build per-shift row map (keyed by shiftId)
    type WorkingRow = {
      shiftId: string;
    ...
    for (const row of rawShiftSalesRows) {
      ...
      workedRows.set(row.shiftId, {
        ...
        scheduledStartMin: null, // filled in during schedule processing
        effectiveLateMinutes: null, // filled in during schedule processing
    ...
    // Process published schedules for attendance + punctuality
    ...
    for (const attendanceUnit of attendanceUnits) {
      ...
      if (worked) {
        ...
        absentRows.push({
          ...
          taskPoints: null,
          compositeScore: round((0 / 15) * 100), // 0 pts out of 15 attendance max = 0
        });
      }
    }
    ...
    const workedScoreRows: ShiftScoreRow[] = Array.from(workedRows.values()).map((row) => {
      ...
      // Attendance: 15 if this shift was scheduled (we know they attended), or null if unscheduled
      const attendancePoints = row.scheduleShiftId != null ? 15 : null;

      // Punctuality: only if scheduled and we have both times
      ...
      // Composite score over available metrics
    ...
    // Combine and sort by date descending, then by started_at descending
