  ...
  const sorted = [...rows].sort((a, b) => {
    ...
    switch (key) {
      ...
      case "score":
        ...
        break;
      case "punctuality":
        va = a.punctualityPoints;
        vb = b.punctualityPoints;
        break;
      case "accuracy":
        va = a.accuracyPoints;
        vb = b.accuracyPoints;
        break;
      case "cash":
        va = a.cashHandlingPoints;
        vb = b.cashHandlingPoints;
        break;
      case "tasks":
        va = a.taskPoints;
        vb = b.taskPoints;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!token) {
          router.replace("/login?next=/scoreboard/shifts");
          return;
        }
        const qs = new URLSearchParams({ from, to, storeId });
        const res = await fetch(`/api/employee/scoreboard/shift-breakdown?${qs.toString()}`, {
        ...
        // Pull scoreboard score from the scoreboard API so the headline score
        // always matches the main scoreboard card exactly.
        try {
          const scoreRes = await fetch(`/api/employee/scoreboard?${qs.toString()}`, {
            headers: { Authorization: `Bearer ${token}` },
          });
          if (scoreRes.ok) {
            const scoreJson = await scoreRes.json();
            if (alive) setScoreboardScore(typeof scoreJson?.myRow?.score === "number" ? scoreJson.myRow.score : null);
          }
        } catch {
          // Keep breakdown usable if the secondary score request fails.
        }
      } catch (e: unknown) {
        if (!alive) return;
    ...
    <div className="app-shell">
      <div className="mx-auto max-w-7xl space-y-4">
        <div className="flex items-center justify-between gap-2">
          <h1 className="text-2xl font-semibold">Shift Breakdown</h1>
          <Link
            href={`/scoreboard?from=${from}&to=${to}&storeId=${storeId}`}
          ...
          <>
            ...
            <div className="card card-pad">
              <div className="text-sm font-semibold mb-2">
                {data.employeeName ?? "My Shifts"} — Shift Breakdown
              </div>
              <div className="flex flex-wrap gap-4 text-sm">
                ...
                {avgScore != null && (
                  <div className="text-zinc-500">
                    Shift Detail Avg: <b className={scoreTone(avgScore)}>{avgScore.toFixed(1)}</b>
                  </div>
                )}
            ...
            {/* Table */}
            {data.rows.length === 0 ? (
              <div className="card card-pad text-sm muted">No shifts found for this period.</div>
            ) : (
                    ...
                    {sortedRows.map((row, i) => (
                      ...
                      >
                        ...
                        <td className="py-2 pr-3 whitespace-nowrap">
                          ...
                          ) : row.scheduledStartMin != null && !row.attended ? (
            ...
            <div className="card card-pad text-xs text-zinc-500 space-y-1">
              ...
              <div>
                Scheduled start time shown as{" "}
                <span className="text-zinc-400">
                  {formatMinutes(9 * 60)} (scheduled)
                </span>{" "}
                format; lateness is actual vs. scheduled in CST.
              </div>
            </div>
          </>
        )}
...
export default function ShiftBreakdownPageWrapper() {
  return (
    <Suspense>
      <ShiftBreakdownPage />
    </Suspense>
